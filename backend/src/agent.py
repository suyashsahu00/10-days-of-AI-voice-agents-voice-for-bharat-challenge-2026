...
class Assistant(Agent):
    ...
    #     # buffer/inspect the text chunk to detect dominant script/language
    #     # simple heuristic: check for Devanagari unicode range
    #     locale_set = False

    #     async def route_locale(text_stream):
    #         nonlocal locale_set
    #         async for chunk in text_stream:
    #             if not locale_set and chunk.strip():
    #                 is_hindi_dominant = any("\u0900" <= ch <= "\u097f" for ch in chunk)
    #                 self.tts.update_options(
    #                     locale="hi-IN" if is_hindi_dominant else "en-IN"
    #                 )
    #                 locale_set = True
    #             yield chunk

    #     async for frame in Agent.default.tts_node(
    #         self, route_locale(text), model_settings
    #     ):
    #         yield frame
